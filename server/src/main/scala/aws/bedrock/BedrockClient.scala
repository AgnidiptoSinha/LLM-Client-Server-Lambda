...
import software.amazon.awssdk.auth.credentials.{AwsBasicCredentials, DefaultCredentialsProvider}
...
class BedrockClient(config: Config) extends LazyLogging {
  ...
  private val credentials = DefaultCredentialsProvider.create()
